...
def split_save_data(config_path, params_path):
    ...
    split_dir = config["artifacts"]["split_data_dir"]
    create_directory([os.path.join(artifacts_dir, split_dir)])
    train_file_name = config["artifacts"]["train"]
    test_file_name = config["artifacts"]["test"]
    train_data_path = os.path.join(artifacts_dir, split_dir, train_file_name)
    test_data_path = os.path.join(artifacts_dir, split_dir, test_file_name)
